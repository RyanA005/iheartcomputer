...
const PROGRAMS = {
    ...
    void main() {
        ...
        if (u_raw > 0.5) {
            ...
            // composite over site paper #f2f2f0 (was white)
            vec3 bg = vec3(0.9490196, 0.9490196, 0.9411765);
            gl_FragColor = vec4(rgba.rgb + (1.0 - rgba.a) * bg, 1.0);
...
export function createCA(gl, layerWeights, gridSize) {
    // gridSize is [width, height] in *simulation* cell units (size = [width, height]).
    // createTensor(h, w) stores {.h=h, .w=w} and uniforms size=[w,h], so pass (height, width).
    ...
    let stateBuf = createTensor(height, width, CHANNEL_N);
    let newStateBuf = createTensor(height, width, CHANNEL_N);
    const perceptionBuf = createTensor(height, width, CHANNEL_N*3);
    const hiddenBuf = createTensor(height, width, 128, 'relu');
    const updateBuf = createTensor(height, width, CHANNEL_N);
    const maskedUpdateBuf = createTensor(height, width, CHANNEL_N);
